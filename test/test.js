...
tape( 'the function throws an error if population size `N` is not a nonnegative integer', function test( t ) {
	var values;
	var i;
...
tape( 'the function throws an error if subpopulation size `K` is not a nonnegative integer', function test( t ) {
	var values;
	var i;
...
tape( 'the function throws an error if number of draws `n` is not a nonnegative integer', function test( t ) {
	var values;
	var i;
...
tape( 'the function throws an error if number of draws `n` is larger than population size `N`', function test( t ) {
	var values;
	var i;
	...
	function badValue( value ) {
		return function badValue() {
			iterator( value[0], value[1], value[2] );
		};
	}
});

tape( 'the function throws an error if subpopulation size `K` is larger than population size `N`', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns an iterator protocol-compliant object (seed)', function test( t ) {
	...
	it1 = iterator( 50, 20, 30, {
		'seed': seed
	});
	it2 = iterator( 50, 20, 30, {
		'seed': seed
	});
...
tape( 'the function returns an iterator for generating pseudorandom numbers from a hypergeometric distribution', function test( t ) {
	var rand;
	var it;
	var i;

	// Note: we assume that the underlying generator is the following PRNG...
	rand = hypergeometric( 10, 4, 6, {
		'seed': 12345
	});
