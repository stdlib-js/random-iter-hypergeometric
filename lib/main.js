...
function iterator( N, K, n, options ) {
	...
	if ( !isNonNegativeInteger( N ) ) {
		throw new TypeError( format( '0Po2p', N ) );
	}
	if ( !isNonNegativeInteger( K ) ) {
		throw new TypeError( format( '0Po3k', K ) );
	}
	if ( !isNonNegativeInteger( n ) ) {
		throw new TypeError( format( '0Po2y', n ) );
	}
	if ( n > N ) {
	...
	if ( arguments.length > 3 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Po2h', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Po35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
