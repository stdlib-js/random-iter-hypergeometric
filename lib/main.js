...
function iterator( N, K, n, options ) {
	...
	if ( !isNonNegativeInteger( N ) ) {
		throw new TypeError( format( '0pp2d', N ) );
	}
	if ( !isNonNegativeInteger( K ) ) {
		throw new TypeError( format( '0pp3X', K ) );
	}
	if ( !isNonNegativeInteger( n ) ) {
		throw new TypeError( format( '0pp2m', n ) );
	}
	if ( n > N ) {
		throw new RangeError( format( '0ppEN', n ) );
	}
	if ( K > N ) {
		throw new RangeError( format( '0ppEO', K ) );
	}
	if ( arguments.length > 3 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pp2V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pp2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
